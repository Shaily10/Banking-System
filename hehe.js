  ...
  const transferMoney = sender => {
    ...
    if (amount > senderObj.balance) {
      ...
      const transferObj = { sender: sender, recipient: recipientName, amount: amount, date: new Date().toLocaleString() };
      senderObj.transfers.push(transferObj);
...
//   const transfers = [  { sender: "Mogli", recipient: "Jaggu", amount: 500 },  { sender: "Bheem", recipient: "Sheru", amount: 1000 },  { sender: "Unkil", recipient: "Chutki", amount: 2000 },  { sender: "Laura", recipient: "Tommy", amount: 3000 },];
...
//     Update the amount column of the row with the matching sender and recipient names
//     const rowsToUpdate = transferList.querySelectorAll(`tr td:nth-child(1):not(:first-child):not(:last-child):not(:empty), tr td:nth-child(2):not(:first-child):not(:last-child):not(:empty)`);
//     rowsToUpdate.forEach((cell) => {
//       const senderName = cell.parentElement.querySelector("td:nth-child(1)").textContent;
//       const recipientName = cell.parentElement.querySelector("td:nth-child(2)").textContent;
//       if (senderName === transfer.sender && recipientName === transfer.recipient) {
//         cell.parentElement.querySelector("td:nth-child(3)").textContent = transfer.amount;
//       }
//     });
//   });
// };
// displayTransfers();
// Call the displayTransfers function to display the transfer history
